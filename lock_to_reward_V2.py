...
pdf = matplotlib.backends.backend_pdf.PdfPages("output.pdf")
for x in range(4):
    cherryx,cherryy,grapex,grapey,bothx,bothy,nox,noy =  new_generate_raster(trial_df, spike_df, x)
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1, projection='scatter_density')
    ax.scatter_density(cherryx, cherryy, color = 'red')
    # ax.scatter_density(grapex, grapey, color = 'purple')
    # ax.scatter_density(bothx, bothy, color = 'blue')
    # ax.scatter_density(nox, noy, color = 'black')
    ax.set_xlim(-1, 5)
