...
The above coefficient plot is the output of my signal detection algorithm. The coefficients show a clear preference to the reward kernel over licking at the time of 0 (which is event time normalised to cover both events) and thus this algorithm is able to classify neuronal cells that encode reward rather than licking. Ultimately, this model allows us to analyse the neural correlates of reward whilst removing licking artefacts.
