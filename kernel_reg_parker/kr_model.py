...
"""Other code to make syth x to probe negative coeff bug"""
#Won't work if uncommented as generates a batch of spike times in one loop with the overall vector needing to match the length variable
# spike_times = []
# for time in reward_times:
#     spike_times.append(np.random.default_rng().normal(loc = time, scale = 0.1, size = 500))
#
# flat_list = [item for sublist in spike_times for item in sublist]
#
# def extractDigits(lst):
#     return [[el] for el in lst]
#
# spike_times = extractDigits(flat_list)
# spike_times = np.asarray(spike_times)
# spike_times = spike_times[0:length]
...
# print("New Design Matrix Shape:", design_matrix.shape)

"""Create new design matrix"""

def create_design_matrix(kernel, naming_int):
        ...
        for i in list: #Starting at 2
            shifted_vector = bin_vector[shift_back + shift_forward + 1 - i:-i]
            design_matrix[str(i - 1 + naming_int)] = shifted_vector #Adds to the dataframe for the size of the kernel window, each column is a param
